<x-app-layout>
    <x-slot name="header">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('public.documents.my_documents') }}</h1>
        <p class="mt-2 text-gray-600 dark:text-gray-400">
            {{ __('public.documents.manage_documents_description') }}
        </p>
    </x-slot>
    ...
    <div class="space-y-8">
        ...
        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-dark-surface dark:shadow-lg dark:border-dark-border">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('public.documents.your_points') }}</h2>
                ...
                <a href="{{ route('documents.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white border border-transparent rounded-md shadow-sm bg-primary-600 dark:bg-dark-button hover:bg-primary-700 dark:hover:bg-dark-buttonHover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-dark-accent dark:focus:ring-offset-dark-bg">
                    ...
                    {{ __('public.documents.upload_document') }}
                </a>
            </div>
        </div>
        ...
        <div>
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">{{ __('public.documents.uploaded_documents') }}</h2>
            @if($uploadedDocuments->isEmpty())
                <div class="p-6 text-center bg-white border border-gray-200 rounded-lg shadow-md dark:bg-dark-surface dark:shadow-lg dark:border-dark-border">
                    ...
                    <p class="mt-4 text-gray-600 dark:text-gray-400">{{ __('public.documents.no_uploaded_documents') }}</p>
                    <a href="{{ route('documents.create') }}" class="inline-flex items-center mt-3 text-sm text-primary-600 dark:text-dark-accent hover:text-primary-500 dark:hover:text-dark-accentHover">
                        {{ __('public.documents.upload_first_document') }}
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full overflow-hidden bg-white border border-gray-200 rounded-lg shadow-md dark:bg-dark-surface dark:shadow-lg dark:border-dark-border">
                        <thead class="border-b border-gray-200 bg-gray-50 dark:bg-dark-header dark:border-dark-border">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.document') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.country') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.format') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.price') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.status') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.downloads') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-dark-border">
                            @foreach($uploadedDocuments as $document)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex px-2 py-0.5 text-xs font-medium rounded-full @if($document->format == 'pdf') bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 @elseif(in_array($document->format, ['doc', 'docx'])) bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 @else bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300 @endif">
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('documents.destroy', $document) }}" onsubmit="return confirm('{{ __('public.documents.confirm_delete') }}');">
                                                @csrf
                                                @method('DELETE')
        ...
        <div>
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">{{ __('public.documents.downloaded_documents') }}</h2>
            @if($downloadedDocuments->isEmpty())
                <div class="p-6 text-center bg-white border border-gray-200 rounded-lg shadow-md dark:bg-dark-surface dark:shadow-lg dark:border-dark-border">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12 mx-auto text-gray-400 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                    </svg>
                    <p class="mt-4 text-gray-600 dark:text-gray-400">{{ __('public.documents.no_downloaded_documents') }}</p>
                    <a href="{{ route('documents.index') }}" class="inline-flex items-center mt-3 text-sm text-primary-600 dark:text-dark-accent hover:text-primary-500 dark:hover:text-dark-accentHover">
                        {{ __('public.documents.browse_catalog') }}
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-1" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M10.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L12.586 11H5a1 1 0 110-2h7.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </a>
                    ...
                    <table class="min-w-full overflow-hidden bg-white border border-gray-200 rounded-lg shadow-md dark:bg-dark-surface dark:shadow-lg dark:border-dark-border">
                        <thead class="border-b border-gray-200 bg-gray-50 dark:bg-dark-header dark:border-dark-border">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.country') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.format') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.price_paid') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.downloaded_at') }}</th>
                                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">{{ __('public.documents.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-dark-border">
                            @foreach($downloadedDocuments as $document)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                        {{ $document->pivot && $document->pivot->downloaded_at ? \Carbon\Carbon::parse($document->pivot->downloaded_at)->format('d/m/Y') : __('public.documents.unknown_date') }}
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
