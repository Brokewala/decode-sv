                    ...
                    @auth
                        @if(auth()->user()->downloadedDocuments()->where('document_id', $document->id)->exists())
                            <div class="p-6 border-t border-gray-200 dark:border-dark-border">
                                ...
                                <form wire:submit.prevent="rateDocument" class="space-y-4">
                                    
                                    <div>
                                        <label for="userRating" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Votre note</label>
                                        <div class="mt-1 flex items-center space-x-2">
                                            <div class="flex space-x-1">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <button type="button" wire:click="$set('userRating', {{ $i }})" class="focus:outline-none">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 {{ $i <= $userRating ? 'text-yellow-500' : 'text-gray-300 dark:text-gray-600' }} hover:text-yellow-500 dark:hover:text-yellow-500" viewBox="0 0 20 20" fill="currentColor">
                                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                                        </svg>
                                                    </button>
                                                @endfor
                                    ...
                                    <div>
                                        <label for="userComment" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Votre commentaire (optionnel)</label>
                                        <div class="mt-1">
                                            <textarea id="userComment" wire:model="userComment" rows="3" 
                                    ...
                                    <div>
                                        <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 dark:bg-dark-button hover:bg-primary-700 dark:hover:bg-dark-buttonHover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-dark-accent dark:focus:ring-offset-dark-bg">
                                            <svg wire:loading wire:target="rateDocument" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
